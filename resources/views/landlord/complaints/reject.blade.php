...
@section('content')
    <div class="container mt-4">
                    ...
                    <div class="d-flex flex-wrap gap-3">
                        @foreach ($complaint->photos as $photo)
                            <a href="{{ asset('storage/' . $photo->photo_path) }}" target="_blank">
                                <img src="{{ asset('storage/' . $photo->photo_path) }}" class="rounded border shadow-sm"
        ...
        <div class="card mb-5">
            <div class="card-body">
                <form action="{{ route('landlord.complaints.reject', $complaint->id) }}" method="POST">
                    @csrf

                    <div class="mb-3">
                        <label for="reject_reason" class="form-label fw-semibold">📝 Lý do từ chối:</label>
                        <textarea name="reject_reason" id="reject_reason" rows="6" class="form-control"
                            placeholder="Vui lòng nhập lý do rõ ràng và cụ thể..." required>{{ old('reject_reason') }}</textarea>
                    </div>

                    <div class="form-check mb-4">
                        <input type="checkbox" class="form-check-input" id="notify_user" name="notify_user" value="1" checked>
                        <label class="form-check-label" for="notify_user">Gửi lý do từ chối cho người thuê</label>
                    </div>
