...
@section('content')
    <div class="container py-5">
                            ...
                            <div class="row g-4">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="number" name="area" id="area"
                                        ...
                                        value="{{ old('area', $post->area) }}" required>
                                    @error('area')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                            ...
                                            <div class="col-md-4">
                                                <div class="form-check">
                                                    <input type="checkbox" name="features[]"
                                                        ...
                                                        {{ in_array($feature->feature_id, old('features', $post->features->pluck('feature_id')->toArray())) ? 'checked' : '' }}>
                                ...
                                <div class="col-12">
                                    <label for="description" class="form-label fw-semibold text-dark">Mô tả chi
                                        tiết</label>
                                    <textarea name="description" id="description" class="form-control shadow-sm rounded-3" rows="8">{{ old('description', $post->description) }}</textarea>
                                    @error('description')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                    ...
                                    <div id="thumbnail-preview" class="mt-3">
                                        @if ($post->thumbnail)
                                            <img id="thumbnail-img" class="img-fluid rounded-3 shadow-sm"
                                                ...
                                                src="{{ asset('storage/' . $post->thumbnail) }}" alt="Thumbnail Preview">
                                        @else
                                            <img id="thumbnail-img" class="img-fluid rounded-3 shadow-sm"
        ...
        <div id="map" class="rounded-3 shadow-sm" style="height: 400px; width: 100%;"></div>
        ...
        <script>
            ...
            const map = new vietmapgl.Map({
                ...
                center: [105.751821, 21.034598], // Tọa độ từ DB
                zoom: 15
            });

            // Add marker
            let marker = new vietmapgl.Marker()
                .setLngLat([105.751821, 21.034598])
                .addTo(map);

            // Check map load and errors
            map.on('load', function() {
                console.log('Bản đồ đã tải thành công');
            });
            map.on('error', function(e) {
                console.error('Lỗi bản đồ:', e);
                alert('Có lỗi khi tải bản đồ. Vui lòng kiểm tra API Key hoặc kết nối mạng.');
            });
            ...
            async function fetchAddress(query) {
                try {
                    ...
                    if (!response.ok) {
                        throw new Error(`Lỗi API LocationIQ: ${response.status}`);
                    }
                    return await response.json();
                } catch (error) {
                    console.error('Error fetching address:', error);
                    alert('Không thể lấy thông tin địa chỉ. Vui lòng kiểm tra kết nối mạng hoặc API Key.');
                    return [];
                }
            }
                ...
                if (provinceCode) {
                    try {
                        ...
                        data.districts.forEach(district => {
                            ...
                            if (district.name === '{{ $post->district }}') {
                                option.selected = true;
                            }
            ...
            async function searchAddress() {
                ...
                if (data.length > 0) {
                    ...
                    alert('Không tìm thấy địa chỉ. Vui lòng thử nhập địa chỉ ngắn hơn hoặc chọn trực tiếp trên bản đồ.');
                }
            }
            ...
            thumbnailInput.addEventListener('change', function(e) {
                ...
                if (file && file.type.startsWith('image/')) {
                    ...
                    thumbnailImg.src = '{{ $post->thumbnail ? asset('storage/' . $post->thumbnail) : '' }}';
                    thumbnailImg.style.display = '{{ $post->thumbnail ? 'block' : 'none' }}';
                }
            });
