...
.navigation.scrolled .dropdown a {
    color: #C62828 !important; /* giữ tone đỏ khi scroll */
}
...
<nav id="navigation" class="navigation navigation-landscape">
        ...
        <ul class="nav-menu nav-menu-social align-to-right">
            @auth
                ...
                <li class="dropdown d-flex align-items-center gap-3 ms-2">
                    ...
                    {{-- Chuông thông báo --}}
    <a href="#" class="position-relative" data-bs-toggle="dropdown" id="notificationDropdown" aria-expanded="false">
    <i class="fas fa-bell fa-lg text-dark"></i>

    @if($unreadCount > 0)
        <span class="position-absolute top-0 start-100 translate-middle badge rounded-circle bg-danger text-white borde"
    ...
    @endif
                    ...
                    <ul class="dropdown-menu dropdown-menu-end shadow-sm" aria-labelledby="notificationDropdown" style="min-width: 300px;">
                                ...
                                <a class="dropdown-item small" href="{{ $n->link }}"
                                   onclick="event.preventDefault(); document.getElementById('nav-read-{{ $n->id }}').submit();">
                                    <div class="@if(!$n->pivot->is_read) fw-bold @endif">
                                        ...
                                        <br><small class="text-muted">{{ \Carbon\Carbon::parse($n->pivot->received_at)->diffForHumans() }}</small>
                                    </div>
                                </a>
                        ...
                        @endforelse
                        <li>
                            <a class="dropdown-item text-center small text-primary fw-bold" href="{{ route('notifications.index') }}">
                                📋 Xem tất cả
                            </a>
                        </li>
                    </ul>
                    ...
                    <a href="#" class="user-dropdown d-flex align-items-center" data-bs-toggle="dropdown">
                        <img src="{{ Auth::user()->avatar ? asset(Auth::user()->avatar) : asset('assets/images/default-avatar.jpg') }}"
                             class="user-avatar rounded-circle" width="32" height="32" alt="avatar" style="object-fit: cover;">
